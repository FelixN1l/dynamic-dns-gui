    ...
    <div>
        ...
        <ul>
          {localIPs.map((ip) => (
            <li key={ip.id}>
              ...
              <button onClick={() => deleteIP(ip.id)} style={{ marginLeft: "10px" }}>Delete</button>
            </li>
          ))}
        </ul>
      )}
      <form onSubmit={addIP}>
        <div>
          ...
          <input type="text" name="ip_address" value={ipForm.ip_address} onChange={handleChange} required />
        </div>
        <div>
          <label>Port: </label>
          <input type="number" name="port" value={ipForm.port} onChange={handleChange} required />
        </div>
